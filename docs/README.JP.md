...
> 完全にTypescriptをサポートし簡単に使うことができる state management 体系です。
...
- **低いランニングカーブ** - 簡単に使える出版-購読型モデル(Pub & Sub)を使用
- **Typescript Intellisense 対応** - <u>ステート</u>、<u>アクション</u>、<u>ミューテーション</u>などを参照する際にエディタによる補完が有効
...
- **柔軟な状態使用許可** - ミューテーションを省くと Getters として使用可能
...
>  `vue-state-store`は、関数またはクラスを通じて、<u>ステート</u>・<u>アクション</u>・<u>ミューテーション</u> の両方を簡単に管理することができます。
...
バインドされた値を変更した場合でも、値が変更されるたびに、その変更は自動的に購読しているコールバック関数により配信されます。
...
> コールバック関数でバインドされたストアの値を `computed` でラップしたコールバック関数で参照すると、ストアの値が変わるたびにコールバック関数が再実行されます。これにより、関数が呼び出されるたびに各関数を再計算する疲労が軽減され、ステートを使用する際のパフォーマンスが向上します。
